    ...
    callback : function (error, result, $) {
        ...
        try {
            ...
            $('body').each(function(index) {
                ...
                //$('tr.td.product-table-display.infos').each(function(index, div) {            //search for additional links
                $('tr td .product-table-display .infos .titre').each(function(index, a) {            //search for additional links
    
                    var composition_regex = new RegExp('([\\w|\\-]+)', 'ig');

                    //console.log("index is: " + index);
                    //console.log("div is: " + $(div).html());

                    buy_now[index] = $(a).attr('href');
                    name[index] = $(a).text();
                    technology[index] = "FDM";
                    website[index] = "http://www.reprap-3d-printer.com/";

                    filament_extrusion_to_flow_multiplier[index] = 1.00;
                    
                    composition[index] = name[index].match(composition_regex)[0];
                    //console.log(composition[index]);

                    if(name[index].match('1.75')){
                        diameter[index] = 1.75;
                    }                    
                    else if (name[index].match('3mm')){
                        diameter[index] = 2.85;
                    }
                    else {
                        diameter[index] = 1.75;
                    ...
                    color[index] = null;
                    opacity[index] = 1;
                    manufacturer[index] = "E Motion Tech";                
                    ...
                    if(name[index].match('ABS')){
                        ...
                        temp_extrude_default[index] = null;
                        temp_extrude_min[index] = null;
                        temp_extrude_max[index] = null;
                        bed_temp[index] = null;
                        bed_material[index] = null;
                        extruder_fan_speed[index] = null;
                        filament_extrusion_to_flow_multiplier[index] = null;
                        min_nozzle_diameter[index] = null;
                    }

                });

                
                $('tr td .product-table-display .infos span.desc').each(function(index, span) { 
                    weight[index] = $(span).text();
                    //console.log(weight[index]);
                });

                $('span.ttc').each(function(index, span) { 
                    price[index] = $(span).parent().text();
...
c.queue(["http://www.reprap-3d-printer.com/category/316-filament-for-3d-printers"]);
